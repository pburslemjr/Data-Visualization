...
server <- function(input, output){
  ...
  #   subset(fullData, 
  #         fullData$race == 'A' & fullData$gender == 'B' & fullData$chronname %in% ) 
  #   
  # %>%
  #         mutate(., raceAndGender = paste(fullData$race,fullData$gender))
  #   })
  
  plotDataLine <- reactive({
    subset(fullData,
           fullData$race == 'A' & fullData$gender == 'B' & fullData$chronname %in% finalValue())
  })
